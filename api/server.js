...
// CORS configuration – allow local dev and the public tunnels (configurable via ALLOWED_ORIGINS env var)
// If ALLOWED_ORIGINS is set, it should be a comma‑separated list of origins.
// Otherwise we fall back to the default list used during development.
const DEFAULT_ORIGINS = [
  'http://localhost:5173',
  'https://loud-flowers-type.loca.lt',
  'https://easy-ends-cross.loca.lt',
  'https://plenty-kings-give.loca.lt'
];
...
http.createServer(async (req, res) => {
  ...
  // Serve static media files (img, gif, media) directly from the media folder
  const staticPrefixes = ['/img/', '/gif/', '/media/'];
  for (const prefix of staticPrefixes) {
    if (url.pathname.startsWith(prefix)) {
      const relPath = decodeURIComponent(url.pathname.slice(prefix.length));
      const filePath = path.join(__dirname, '..', 'media', relPath);
